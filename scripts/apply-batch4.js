...
const batch4 = JSON.parse(fs.readFileSync('scripts/all-exercises-batch4.json', 'utf8'));

console.log('应用批次4练习题...\n');
let totalAdded = 0;

data.data.forEach(skill => {
  if (batch4[skill.skillId]) {
    const before = skill.advancedExercises.length;
    skill.advancedExercises.push(...batch4[skill.skillId]);
